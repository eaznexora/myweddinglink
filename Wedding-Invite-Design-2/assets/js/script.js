...
function showStickyNav() {
    ...
    scrollContainer.addEventListener('scroll', () => {
        ...
        if (currentScrollTop < lastScrollTop) {
            // Scrolling UP — show nav and music btn
            nav.classList.remove('nav-hidden');
            nav.classList.add('nav-visible');
            if (musicBtn && currentScrollTop < 100) {
                musicBtn.style.opacity = '1';
                musicBtn.style.pointerEvents = 'auto';
                musicBtn.style.transform = 'scale(1)';
            }
        } else {
            // Scrolling DOWN — hide nav and music btn
            nav.classList.remove('nav-visible');
            nav.classList.add('nav-hidden');
            if (musicBtn && currentScrollTop > 50) {
...
// ============================================
// ADD TO CALENDAR FUNCTIONALITY (Direct Open)
// ============================================
function addToCalendar(eventConfig) {
    ...
    if (isIOS) {
        // iOS: ICS data URI opens directly in Apple Calendar without a downloads folder
        const icsMSG = `BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//EazSocial//Wedding Invite//EN\nBEGIN:VEVENT\nUID:${new Date().getTime()}@eazsocial.com\nDTSTAMP:${new Date().toISOString().replace(/[-:]/g, '').split('.')[0]}Z\nDTSTART:${startDate}\nDTEND:${endDate}\nSUMMARY:${title}\nDESCRIPTION:${description}\nLOCATION:${location}\nBEGIN:VALARM\nTRIGGER:-PT5H\nACTION:DISPLAY\nDESCRIPTION:Reminder\nEND:VALARM\nEND:VEVENT\nEND:VCALENDAR`;
        window.location.href = 'data:text/calendar;charset=utf-8,' + encodeURIComponent(icsMSG);
    } else {
        // Android & Desktop: Open Google Calendar directly (avoids annoying .ics file downloads)
...
function playBackgroundMusic() {
    ...
    if (bgMusic) {
        bgMusic.play().then(() => {
            ...
            if (musicBtn) {
                musicBtn.classList.remove('opacity-0', 'pointer-events-none', 'scale-90');
                musicBtn.classList.add('opacity-100', 'pointer-events-auto', 'scale-100');
                isMusicIconVisible = true;
            }
        }).catch(err => {
            console.log('Autoplay prevented by browser', err);
            // Even if autoplay is prevented, show the button so user can click it to play
            if (musicBtn) {
                musicBtn.classList.remove('opacity-0', 'pointer-events-none', 'scale-90');
                musicBtn.classList.add('opacity-100', 'pointer-events-auto', 'scale-100');
                isMusicIconVisible = true;
