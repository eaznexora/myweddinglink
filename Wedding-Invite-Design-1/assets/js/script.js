...
function initMusic() {
    ...
    if (bgMusic && musicToggleBtn && !hasMusicStarted) {
        bgMusic.volume = 0.5;
        // Attempt to autoplay
        bgMusic.play().then(() => {
            isMusicPlaying = true;
            hasMusicStarted = true;
            musicToggleBtn.classList.remove('opacity-0', 'pointer-events-none');
        }).catch(e => {
            console.log('Audio autoplay prevented by browser. Waiting for user interaction.', e);
            hasMusicStarted = true;
            musicToggleBtn.classList.remove('opacity-0', 'pointer-events-none');
            const musicIcon = document.getElementById('musicIcon');
            if(musicIcon) {
                // Set to off icon since it failed to autoplay
                musicIcon.innerHTML = '<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line>';
            }
        });
    }
...
function scaleSections() {
    ...
    sectionsToScale.forEach((sec) => {
        ...
        if (inner && section) {
            ...
            // Fix the outer section height so it doesn't overlap or leave gaps
...
function showStickyNav() {
    ...
    scrollContainer.addEventListener('scroll', () => {
        ...
        // Hide music button when scrolling down past 100px
        if (musicBtn) {
            if (currentScrollTop > 100) {
                musicBtn.classList.add('opacity-0', 'pointer-events-none');
            } else {
